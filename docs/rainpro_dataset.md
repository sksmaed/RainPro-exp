...
- 欄位：max dBZ、time、經緯度
- 哨兵值（**兩者語意不同，不可混為一談**，見 `rainpro8_sources.QPESUMS_MISSING_VALUES` /
  `QPESUMS_NO_ECHO_VALUES`）：
  - **-999 = 真正未觀測** → 轉成 NaN，排除於 loss 之外
  - **-99 = 無回波**（有觀測，只是低於雷達偵測門檻）→ 視為 **0 dBZ**，是有效的低反射率監督訊號
- 實測佔比（真實 store）：-99 約佔每張影像的 **98.4%**，-999 在抽樣的影像中為 0%
- 判定依據：-99 的遮罩**隨天氣變化**，不是固定的地理覆蓋遮罩 —— 相隔數年的兩個時間點只有
  87.6% 重疊，而且它的形狀會跟著回波移動。論文 Table 8 也佐證這個分類：訓練集有 **79.64%**
  的像素落在最低的「無雨」bucket 且**是有監督的類別**，missing 只佔 12.97%。把 -99 當成缺測
  會讓台灣版的 missing 比例變成 98.4%，與論文設計完全不符。
