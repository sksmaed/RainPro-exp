...
- **GFS channels**: `--data.include_gfs true` defaults to the paper's 122-channel App. I selection (`GFS_ANALYSIS_VARIABLES` in `rainpro/data/rainpro8_sources.py`), flattened to one canonical `VAR` or `VAR_level` name per channel (e.g. `TMP_850mb`). These are canonical names, not necessarily the real GFS zarr store's own field names -- map them via `data.variable_aliases` if the store differs.
- **GFS normalization**: only sources with well-known physical ranges (radar dBZ, IR brightness temperature) have default min-max bounds (`rainpro/data/normalize.py`). GFS variables (`--data.include_gfs true`) are left un-normalized until per-variable training-set statistics are computed and passed via `data.norm_bounds`.
